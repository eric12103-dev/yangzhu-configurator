...
// ── 主要函式二：純重算刀模（去背完成後重新指定邊距）──────────────────────
// 傳入：已去背的 DataURL + 新的邊距（px）
// 回傳：{ success: bool, overlayDataURL: string, error: string }

async function calcContourOnlyClient(rembgDataURL, marginPx) {
  var marginMm = Math.max(1, Math.round(marginPx / 5));
  try {
    var blob = _dataURLtoBlob(rembgDataURL);
    var result = await _callPreviewDie(blob, marginMm, null);
    return {
...
async function _callPreviewDie(imageBlob, marginMm, onProgress) {
  try {
    var formData = new FormData();
